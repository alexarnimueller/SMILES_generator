...
def main(_):
    ...
    model = SMILESmodel(dataset=FLAGS.dataset, num_epochs=FLAGS.epochs_to_train, run_name=run,
                        reinforce=FLAGS.reinforce, batch_size=FLAGS.batch_size, validation=FLAGS.validation,
    ...
    valid_mols = model.sample_points(FLAGS.n_sample, FLAGS.temp)
    mol_file = open('./generated/' + run + '_finetuned.csv', 'a')
    mol_file.write("\n".join(valid_mols))
    print("Valid:\t{}/{}".format(len(valid_mols), FLAGS.n_sample))
