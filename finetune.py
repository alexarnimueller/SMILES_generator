...
flags = tf.app.flags
flags.DEFINE_string("model_path", "checkpoint/chembl24_100k/", "path (folder) of the pretrained model")
...
flags.DEFINE_integer("num_sample", 50, "number of points to sample from trained model")
flags.DEFINE_float("temp", 1.0, "temperature to sample at")
flags.DEFINE_integer("sample_after", 5, "sample after how many epochs (if 0, no sampling)")
...
def main(_):
    ...
    model.load_data(preprocess=FLAGS.preprocess, stereochem=FLAGS.stereochemistry)
    model.load_model_from_file(FLAGS.model_path, FLAGS.epoch_to_load)
    print("Pre-trained model loaded, finetuning...")

    model.train_model(n_sample=10)
